...
public class Main
{
	...
	private JButton exportexcel = new JButton("导出至Excel文件");
	...
	public static String outputfilename_time;
	...
	public static boolean isWrongFileName;
...
/*		Vector<String> inoutselection = new Vector<String>();
		...
		//初次启动时即启动定时器
		String strManner = ConfigFile.getImportManner();
	...
	public static void outquerybtnClicked() {
		...
		if (!ConfigFile.getImportManner().equals("hour"))
		{
		...
		try
		{
					...
					String query = "select alias.Name,alias.ID,alias.Signal,alias.Location ,statistics_out.Time,statistics_out.Assgn,statistics_out.Avlb,statistics_out.Seiz, "
							...
							+ "WHERE UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(statistics_out.Time)<=3600 and UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(statistics_out.Time)>=0 and statistics_out.ID=alias.ID ;";
					//查询用户选择的数据表
	...
	public static void inquerybtnClicked(){
		...
		try
		{
			...
			String query = "select alias.Name,alias.ID,alias.Signal,alias.Location ,statistics_inc.Time,statistics_inc.Assgn,statistics_inc.Avlb,statistics_inc.Seiz, "
					...
					+ "WHERE UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(statistics_inc.Time)<=3600 and UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(statistics_inc.Time)>=0 and statistics_inc.ID=alias.ID ;";
			//查询用户选择的数据表
	...
	public static void conquerybtnClicked()
	{
		...
		try
		{
				...
				{
					ID = (String)IDQuery.getSelectedItem();
					String TimeSel = "";
					//此处设定选择的时间范围，一天三天或者一周
					if (timeselected.getSelectedIndex() == 1)
					{
						outputfilename_time = "最近一天";
						TimeSel = " WHERE UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(statistics_inc.Time)<=86400 ";
					}
					else if (timeselected.getSelectedIndex() == 2)
					{
						outputfilename_time = "最近三天";
						TimeSel = " WHERE UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(statistics_inc.Time)<=259200 ";
					}
					else if (timeselected.getSelectedIndex() == 3)
					{
						outputfilename_time = "最近一周";
						TimeSel = " WHERE UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(statistics_inc.Time)<=604800 ";
					}
					else if (timeselected.getSelectedIndex() == 4)
					{
						outputfilename_time = "最近一月";
						TimeSel = " WHERE UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(statistics_inc.Time)<=2592000 ";
					}
					else
					{
				...
				else if (inoutselected.getSelectedIndex() == 2)
				{
					...
					{
						TimeSel = " WHERE UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(statistics_out.Time)<=604800 ";
					}
					else if (timeselected.getSelectedIndex() == 4)
					{
						TimeSel = " WHERE UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(statistics_out.Time)<=2592000 ";
					}
					else
					{
						JOptionPane.showMessageDialog(null, "请选择查询时间！", "警告", JOptionPane.INFORMATION_MESSAGE);
	...
	public static void exportexcelbtnClicked()
	{
		if(conrs != null)
			DB2Excel(conrs);
		else 
	...
	public static boolean DB2Excel(ResultSet rs)
	  {
	        ...
	        try {
	        	...
	        	fos = new FileOutputStream("局向"+ID+outputfilename_time+isinout+"话务量统计.xls");
	            workbook = Workbook.createWorkbook(fos);
	             
	             // 创建Excel表中的sheet
	            sheet = workbook.createSheet("话务量统计", 0);
	            ...
	            if (inoutselected.getSelectedIndex() == 1)
	            {
	            	String[] columnTitle = {"局向名称","局向ID","中继方式","局向类型","统计时间","中继分配条数","中继可用条数","占用次数","应答次数","占用话务量","通话话务量","落地试呼次数","转话试呼次数"
	    					,"落地应答次数","转话应答次数"};
	            ...
	            {
	            	String[] columnTitle = {"局向名称","局向ID","中继方式","局向类型","统计时间","中继分配条数","中继可用条数","占用次数","应答次数","占用话务量","通话话务量","试呼次数","接通次数"
							,"中继重选次数","无应答次数","被叫忙次数"};
	            	 for (int i = 0; i < columnCount; i++) 
		            	{
			                colName = columnTitle[i];
	            ...
	            do{
	                for (int i = 0; i < columnCount; i++) {
	                    ...
	                    //WriteLog.WriteLogFile("行:"+i+"---"+row +"---"+ rs.getString(i+1));
	                    sheet.addCell(label);
	                }
	                row++;
	            }while (rs.next()) ;
	...
	public static void ImportTrafficManner(String str,Boolean isChanged) 
	{
        ...
        String trafficPath = ConfigFile.getTrafficDir();
		String logFilename =trafficPath.concat(tinyyear+"").concat(strMonth).concat(strDay).concat("A.LOG");              
		if (str.equals("day"))
		{
			timerSelected = 0;
	        //定制每天的00:10:00执行，
		...
		else if (str.equals("manual"))
		{
			if(isChanged)
			{
				DataImportDiag.beginTimeInt = 0;
				DataImportDiag.endTimeInt = 0;
				timerSelected = 2;
				...
				{
					DataImportDiag.ThreadEndFlag = 1;
					isWrongFileName = true;
					tbLabel.setText("打开文件失败！");
				}
				else{
